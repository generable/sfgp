  ...
  public = list(
    ...
    parse_term = function(idx) {
      ...
      if (pt$type == "gp") {
        ...
        out <- create_sfterm(pt$covariates, pt$hierarchy)
      } else if (pt$type == "sff") {
        out <- create_sffterm(pt$covariates, pt$hierarchy)
      } else if (pt$type == "emax") {
        out <- EmaxTerm$new(
          x_name = pt$covariates[1],
          z_name = pt$covariates[2]
        )
      } else {
        stop("allowed terms are gp(), sf(), sff(), emax() or offset()!")
      }
      return(out)
    },
