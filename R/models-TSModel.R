...
# Helper
sigma_star_lognormal <- function(p) {
  0.5 * (log(1 + p) - log(1 - p))
}

#' Create a prior of noise parameter based on information about expected
#' amount of measurement error
#'
#' @export
#' @param p A priori the most likely value of measurement error percentage
#' @param p_sd Prior std in measurement error percentage
prior_sigma_informed <- function(p = 0.065, p_sd = 0.015) {
  checkmate::assert_number(p, lower = 0, upper = 1)
  checkmate::assert_number(p_sd, lower = 0, upper = 1)
  sigma_mean <- sigma_star_lognormal(p)
  ss_up <- sigma_star_lognormal(p + p_sd)
  ss_low <- sigma_star_lognormal(p - p_sd)
  diffs <- c(sigma_mean - ss_up, sigma_mean - ss_low)
  sigma_sd <- mean(abs(diffs))
  m <- round(sigma_mean, 5)
  s <- round(sigma_sd, 5)
  paste0("normal(", m, ", ", s, ")")
}
